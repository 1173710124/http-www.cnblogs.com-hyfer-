...
public class TrailBullet : MonoBehaviour 
{
    ...
    void OnTriggerEnter2D(Collider2D col)  //撞boss自毁
    {
        ...
        {
            StageController.ScInstantce.AddScore("TrailBullet");//加分
    ...
    void Start()
    {
        ...
        destroytime = Time.time + StageController.ScInstantce.FloPlayerBulletDestroyTime;
    }
    ...
    private float VectorAngle(Vector2 from, Vector2 to)//计算从向量A到向量B的旋转角
    {
        float angle;
